...
// Parse UTC offset in decimal hours from a timezone string like "CET (UTC+1)" or "IST (UTC+5:30)"
...
// Get the local Date in a given UTC offset
function timeAtOffset(offset) {
  const utcMs = Date.now() + new Date().getTimezoneOffset() * 60000;
  return new Date(utcMs + offset * 3600000);
}

// Returns array of 24 booleans: is this UTC hour overlapping working time (9-17 local) for both offsets?
function getOverlapGrid(off1, off2) {
...
📌 Programme: SAP Next Gen Connections Passport`;
  ...
  function MatchRow({ m }) {
    ...
    // Timezone data
    const myTz = currentUser.timezone || "GMT (UTC+0)";
    const theirTz = other.timezone || "GMT (UTC+0)";
    const myOff = parseOffset(myTz);
    const theirOff = parseOffset(theirTz);
    const overlapGrid = getOverlapGrid(myOff, theirOff);
    const overlapHours = overlapGrid.filter(h => h.both).length;
    const myNow = timeAtOffset(myOff);
    const theirNow = timeAtOffset(theirOff);
        ...
        {m.status === "active" && tzOpen && (
          <div className="mx-4 mb-3 rounded-xl p-3 space-y-3"
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: "You", name: currentUser.name.split(" ")[0], tz: myTz, now: myNow, color: "#1B90FF" },
                { label: other.name.split(" ")[0], name: other.office, tz: theirTz, now: theirNow, color: "#DF1278" },
              ].map(p => (
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-semibold text-[#002060] mb-2"
            style={{ fontFamily: "'Fraunces', serif" }}>
            {editMode ? "Edit your profile" : "Join Connections Passport"}
          </h1>
          <p className="text-sm text-[#445063]">
            {editMode
              ? "Update your details and interests — changes apply to future matches."
              : "Connect with Early Talent across SAP's global offices — one chat at a time."}
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-white p-6 space-y-4" style={{ border: "1px solid #CFE6FA" }}>
              ...
              <div className="rounded-xl p-4 space-y-3 text-xs text-[#445063] leading-relaxed"
                ...
                <p><span className="font-semibold text-[#002060]">What we collect:</span> Your name, SAP role, office location, and interests — used solely to match you with other Early Talent participants.</p>
                ...
                <p><span className="font-semibold text-[#002060]">How long we keep it:</span> Your data is retained for the duration of your participation. You can pause or permanently delete your account at any time from your Profile page.</p>
